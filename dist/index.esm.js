...
var calculatePosition = function (triggerElement, menuElement, offset) {
    ...
    var top = spaceBelow >= menuRect.height + offset || spaceBelow >= spaceAbove
...
var Dropdown = function (_a) {
    ...
    var filteredOptions = filterOptions(options, searchTerm);
    ...
    var renderSingleOption = function (option, index) {
        ...
        return (jsxs("div", { ref: function (el) { return (optionRefs.current[index] = el); }, className: clsx('px-3 py-2 cursor-pointer flex items-center justify-between hover:bg-gray-50 transition-colors', {
                'bg-blue-50': isFocused && !isSelected,
                'bg-blue-100': isSelected && !multiple,
                'bg-green-100': isSelected && multiple,
                'opacity-50 cursor-not-allowed': option.disabled,
            }), onClick: function () { return !option.disabled && handleOptionSelect(option); }, onMouseEnter: function () { return setFocusedIndex(index); }, children: [jsxs("div", { className: 'flex items-center flex-1', children: [option.icon && jsx("span", { className: 'mr-2', children: option.icon }), jsxs("div", { children: [jsx("div", { className: 'font-medium', children: option.label }), option.description && (jsx("div", { className: 'text-sm text-gray-500', children: option.description }))] })] }), multiple && isSelected && jsx(CheckIcon, {})] }, option.value));
    };
    var renderOptionsList = function () {
        if (loading) {
            return (jsxs("div", { className: 'px-3 py-2 text-center text-gray-500', children: [jsx(LoadingIcon, {}), jsx("span", { className: 'ml-2', children: loadingText })] }));
        }
        if (filteredOptions.length === 0) {
            return (jsx("div", { className: 'px-3 py-2 text-center text-gray-500', children: "No results found" }));
        }
        return filteredOptions.map(function (option, index) {
            return renderSingleOption(option, index);
        });
    };
    var menu = (jsxs("div", { ref: menuRef, className: 'absolute bg-white border border-gray-200 rounded-md shadow-lg overflow-hidden animate-scale-in', style: {
            top: menuPosition.top,
            left: menuPosition.left,
            minWidth: menuPosition.width,
            zIndex: zIndex,
        }, children: [searchable && (jsx("div", { className: 'p-2 border-b border-gray-200', children: jsxs("div", { className: 'relative', children: [jsx("div", { className: 'absolute inset-y-0 left-0 pl-2 flex items-center pointer-events-none', children: jsx(SearchIcon, {}) }), jsx("input", { ref: searchRef, type: 'text', className: 'w-full pl-8 pr-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none', placeholder: searchPlaceholder, value: searchTerm, onChange: function (e) { return setSearchTerm(e.target.value); }, onKeyDown: handleKeyDown })] }) })), jsx("div", { className: 'max-h-48 overflow-y-auto dropdown-scrollbar', children: renderOptionsList() })] }));
